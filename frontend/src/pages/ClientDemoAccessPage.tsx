      ...
      try {
        ...
        setError(
          loginError instanceof Error
            ? loginError.message
            : 'Shared demo login failed. Please retry in a moment.'
        );
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="site-shell flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-3xl rounded-[2.4rem] border border-white/10 bg-slate-950/76 p-8 shadow-[0_50px_150px_-70px_rgba(0,0,0,0.95)] sm:p-10">
        ...
        <h1 className="mt-6 text-4xl font-semibold tracking-tight text-white sm:text-5xl">
          Enter the live AI calling demo.
        </h1>
        <p className="mt-5 max-w-2xl text-base leading-8 text-slate-300 sm:text-lg">
          This page signs into the shared demo account and takes the client straight to the AI Customer Assistant dashboard.
        </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="text-lg font-semibold text-white">
                {isLoading ? 'Signing into the shared demo account...' : error ? 'Demo access needs attention' : 'Redirecting to the dashboard...'}
              </div>
